...
class BuscarEstoquesQuantidadeBaixaTest {
    @InjectMocks
    private BuscarEstoquesQuantidadeBaixa estoqueService;

    @Mock
    private EstoqueRepository estoqueRepository;

    @Test
    public void testGetFornecedoresComEstoqueBaixo() {
        ...
        when(estoqueRepository.findFornecedoresComEstoqueBaixo()).thenReturn(resultadoEsperado);

        List<Map<String, Object>> fornecedoresComEstoqueBaixo = estoqueService.getFornecedoresComEstoqueBaixo();

        assertNotNull(fornecedoresComEstoqueBaixo);

        assertFalse(fornecedoresComEstoqueBaixo.isEmpty());

        Map<String, Object> primeiroItem = fornecedoresComEstoqueBaixo.get(0);
